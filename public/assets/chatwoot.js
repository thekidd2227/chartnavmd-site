/**
 * ARCG Systems — Chatwoot website chat widget loader
 *
 * Single source of truth for the chat widget on arcgsystems.com.
 * To change the Chatwoot instance or inbox, edit the two values below.
 *
 * Setup:
 *   1. Sign in at https://app.chatwoot.com (account 160989).
 *   2. Create a Website inbox for arcgsystems.com if one does not exist yet.
 ...
 *   6. Push this file — every page loads it via the root index.html.
 *
 * Behavior notes
...
(function () {
  // ──────────────────────────────────────────────
  // CONFIG — edit these two values after Chatwoot inbox setup
  var CHATWOOT_TOKEN = 'VP1Hx2nYthJzHpteEWxDbaK8';
  ...
  // Bail silently if the token hasn't been set yet.
  if (!CHATWOOT_TOKEN || CHATWOOT_TOKEN === 'PASTE_YOUR_WEBSITE_TOKEN_HERE') {
    console.info('[ARCG] Chatwoot widget not loaded — website token not configured yet. See /assets/chatwoot.js.');
    return;
  }

  // ── Size override — injected once, overrides Chatwoot's defaults ──
  function injectSizeCss() {
    if (document.getElementById('arcg-cw-size-css')) return;
    var style = document.createElement('style');
    style.id = 'arcg-cw-size-css';
    style.textContent = [
  ...
  g.onload = function () {
    ...
    // Inject size overrides as soon as the holder appears.
